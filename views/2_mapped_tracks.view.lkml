view: mapped_tracks {
  derived_table: {
    ...
    sql: select *
        ,timestamp_diff(cast(timestamp as timestamp), cast(lag(timestamp) over(partition by looker_visitor_id order by timestamp) as timestamp), minute) as idle_time_minutes
      from (
